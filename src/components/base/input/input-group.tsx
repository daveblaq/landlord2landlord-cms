...
export const InputPrefix = ({ isDisabled, children, ...props }: InputPrefixProps) => (
    <span
        {...props}
        className={cx(
            "flex text-sm text-tertiary shadow-xs ring-1 ring-border-primary ring-inset",
            ...
            "in-data-input-wrapper:in-data-[input-size=lg]:py-2.5 in-data-input-wrapper:in-data-[input-size=lg]:px-4 in-data-input-wrapper:in-data-[input-size=md]:py-2 in-data-input-wrapper:in-data-[input-size=md]:pr-3 in-data-input-wrapper:in-data-[input-size=md]:pl-3.5 in-data-input-wrapper:in-data-[input-size=sm]:px-3 in-data-input-wrapper:in-data-[input-size=sm]:py-2.5",
            // Disabled styles
    ...
    const paddings = sortCx({
        ...
        lg: {
            input: cx(
                ...
                hasTrailing && (prefix ? "group-has-[&>select]:pr-6.5 group-has-[&>select]:pl-0" : "group-has-[&>select]:pr-6.5 group-has-[&>select]:pl-3.5"),
            ),
            leadingText: "pl-4",
        },
    });
                    ...
                    >
                        ...
                        {prefix && (
                            <span className={cx("my-auto grow pr-2", paddings[size].leadingText)}>
                                <p className={cx("text-sm text-tertiary", isDisabled && "text-disabled")}>{prefix}</p>
                            </span>
                        )}
